...
for(i in 1:length(player_info)){
  player_info[[i]]$heights <- heights[i]
}
...
for(i in names(player_info)){
  ...
  points_per_game[[i]] <- individual_points
}
...
```{r}
game_vec <- sapply(points_per_game, function(x) {round(x, 2)})
...
round_fun <- function(my_int){
  ...
  } else if(my_int >= 100 && my_int < 1000){
...
max_temp_index <- which.max(annual_temps)
min_temp_index <- which.min(annual_temps)
...
sum_fun <- function(my_int){
  if(my_int == 0){
    ...
    last_digit = my_int %% 10
    number_truncated <- my_int %/% 10
    return(last_digit + sum_fun(number_truncated))
  }
}
