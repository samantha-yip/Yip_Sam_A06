...
[https://github.com/samantha-yip/Yip_Sam_A06](https://github.com/samantha-yip/Yip_Sam_A06)
...
```{r}
heights <- c(78, 81, 78, 86, 85) # create new heights vector 

for(i in 1:length(player_info)){ #iterate through player_info
  player_info[[i]]$heights <- heights[i] # use indexing to add a new element named heights in player_info
}
...
```{r}
points_per_game <- list() # initialize an empty list

for(i in names(player_info)){ # iterate through names
  # calculate individual point calculations for each player using indexing
  individual_points <- (player_info[[i]]$points / player_info[[i]]$games_played) 

  points_per_game[[i]] <- individual_points # add individual points to list
}
...
```{r}
game_vec <- sapply(points_per_game, function(x) {round(x, 2)}) # use sapply to store output as a vector
...
round_fun <- function(my_int){ # write round function to put inside apply
  if(my_int < 100){ # if value has 2 digits
    round(my_int, digits = -1)
  } else if(my_int >= 100 && my_int < 1000){ # if value has 3 digits
    round(my_int, digits = -2)
  } else if(my_int >= 1000){ # if value has 4 digits
    round(my_int, digits = -3)
  }
}


apply(mat_rand, MARGIN = 1:2, round_fun) # use apply function 
...
```{r}
month_means <- apply(max_temps[,3:14], 2, mean) # call apply on columns 3-14 
month_means
```
...
```{r}
annual_temps <- apply(temps_CO[,3:14], 1, median) # create matrix to store median annual temps 
max_temp_index <- which.max(annual_temps) # store the index of the max annual temp
min_temp_index <- which.min(annual_temps) # store the index of the min annual temp

print(paste0("The maximum median annual temperature of ", annual_temps[max_temp_index], " occured in ", temps_CO$year[max_temp_index], " and the minimum median annual temperature of ", annual_temps[min_temp_index], " occured in ", temps_CO$year[min_temp_index]))
```
...
```{r}
sum_fun <- function(my_int){
  if(my_int == 0){ # base case
    return(0)
  } else{
    last_digit = my_int %% 10 # get the last digit of the number
    new_num <- my_int %/% 10 # get the number without the last digit 
    return(last_digit + sum_fun(new_num))
  }
}
